...
function get_info(){
	...
	$d->query($sql);
	$item = $d->fetch_array();
	$ds_photo= _result_array("select * from #_baiviet_photo where id_info='".$item['id']."' and type='".$_GET['type']."-photo' order by stt desc ");
}
...
function save_info(){
	...
	$d->reset();
	$sql = "select count(id) as num,id from table_info where type like '".$_GET['type']."'";	
	$d->query($sql);
