...
function get_gioithieu(){
	...
	if($_REQUEST['xoaanh']!='')
	{
		...
		$spdc=time();
		$sql_sp = "SELECT id,photo FROM table_bgweb WHERE type='".$type."'";
		$d->query($sql_sp);
		$cats= $d->result_array();
		$spdc1=$cats[0]['photo'];
		if($spdc1!='')
...
function save_gioithieu(){
	...
	$d->reset();
	$sql = "select * from #_bgweb where type like '".$_GET['type']."' ";	
	$d->query($sql);
	...
	if(count($row_item)>0){
		...
		$data['fix_bg'] = isset($_POST['fix_bg']) ? 'fixed' : '';
