...
function GetLink_HTTPS($url){
	...
	$sql = "select ten_vi,id,stt,hienthi,id_list,id_cat,noibat,id_item,id_sub,photo,banchay,footer,id_linhvuc from $where $limit";
	$d->query($sql);
...
function get_man(){
	...
	$sql = "select * from #_baiviet_photo where id_baiviet='".$id."' and type='".$_GET['type']."' order by stt asc ";
	$d->query($sql);
...
function save_man(){
	...
	if($_GET['type'] != 'goctuvan'){
		$files_type = "jpg|png|gif|JPG|jpeg|JPEG";
	}else {
		$files_type = "doc|docx|xls|xlsx";
	}	
	...
	if($id){
		$data['ngaysua'] = time();
		if($photo = upload_image("file_2", $files_type, _upload_baiviet,$file_name_2)){
			$data['photo_2'] = $photo;
			$data['thumb_2'] = create_thumb($data['photo_2'], 1200, 500, _upload_baiviet,$file_name_2,1);
			$d->setTable('baiviet');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
		...
		if($photo = upload_image("file", $files_type, _upload_baiviet,$file_name)){
			$data['photo'] = $photo;
			if($_GET['type'] != 'goctuvan'){
		...
		if($photo = upload_image("file_2", $files_type, _upload_baiviet,$file_name)){
			$data['photo_2'] = $photo;
			$data['thumb_2'] = create_thumb($data['photo_2'], 1200, 500, _upload_baiviet,$file_name,1);
		}	
...
function get_lists(){
	...
	$sql = "select ten_vi,id,stt,hienthi,noibat,photo,footer,id_linhvuc from $where $limit";
	$d->query($sql);
	$items = $d->result_array();
...
function get_cats(){
	...
	$sql = "select ten_vi,id,stt,hienthi,id_list,noibat,photo,id_linhvuc from $where $limit";
	$d->query($sql);
	$items = $d->result_array();
