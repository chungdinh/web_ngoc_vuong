...
function delete(){
	...
	if(isset($_GET['id'])){
		...
		$d->query($sql);
		if($d->query($sql))
			redirect("index.php?com=newsletter&act=man&type=email");
		else
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=newsletter&act=man&type=email");
	}else transfer("Không nhận được dữ liệu", "index.php?com=newsletter&act=man&type=email");		
}
