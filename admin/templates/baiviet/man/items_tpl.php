...
<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
            <li><a href="index.php?com=baiviet&act=man<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>"><span>Quản lý
        ...
        <div class="table-responsive">
            <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
                <thead>
                    <tr>
                        <td></td>
                        <td class="tb_data_small"><a href="#" class="tipS" style="width: auto;">Thứ tự</a></td>
                        ...
                        <?php } ?>
                        <?php if($config_ten){ ?>
                        <td class="sortCol post_name">
                            <div>Tên bài viết<span></span></div>
                        </td>
                        ...
                        <td class="tb_data_small">
                            <?php echo $config_text_banchay ?>
                        </td>
                        <?php } if($config_footer){ ?>
                        <td class="tb_data_small">
                            <?php echo $config_text_footer ?>
                        </td>
                        <?php } if($_GET['type'] != 'slogan2'){ ?>
                        <td class="tb_data_small">Ẩn/Hiện</td>
                        <td style="width:auto">Thao tác</td>
                        <?php } ?>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="title_name_data post_name">
                            <a href="index.php?com=baiviet&act=edit&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id_item=<?=$items[$i]['id_item']?>&id_sub=<?=$items[$i]['id_sub']?>&id=<?=$items[$i]['id']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" class="tipS SC_bold">
                                <?=$items[$i]['ten_vi']?></a>
                        </td>
                        <?php } if($config_images){ ?>
                        <td class="title_name_data" style="width: 90px; text-align: center;">
                            <a href="index.php?com=baiviet&act=edit&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id_item=<?=$items[$i]['id_item']?>&id_sub=<?=$items[$i]['id_sub']?>&id=<?=$items[$i]['id']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" class="tipS SC_bold">
                                <img src="<?php echo _upload_baiviet.$items[$i]['photo'] ?>" alt="" style="width: 90px;">
                            </a>
                        </td>
                        <?php } if($config_noibat){ ?>
                        <td align="center">
                        ...
                        <?php } if($config_banchay){ ?>
                        <td align="center">
                            <a data-val2="table_<?=$_GET['com']?>" rel="<?=$items[$i]['banchay']?>" data-val3="banchay" class="diamondToggle <?=($items[$i]['banchay']==1)?" diamondToggleOff":""?>" data-val0="
                                <?=$items[$i]['id']?>" ></a>
                        </td>
                        <?php } if($config_footer){ ?>
                        <td align="center">
                            <a data-val2="table_<?=$_GET['com']?>" rel="<?=$items[$i]['footer']?>" data-val3="footer" class="diamondToggle <?=($items[$i]['footer']==1)?" diamondToggleOff":""?>" data-val0="
                                <?=$items[$i]['id']?>" ></a>
                        </td>
                        <?php } if($_GET['type'] != 'slogan2'){ ?>
                        <td align="center">
