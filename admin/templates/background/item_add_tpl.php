<form name="supplier" id="validate" class="form" action="index.php?com=background&act=save<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
    <div class="widget">
        <div class="control_frm" style="margin-top:25px;">
            <div class="bc">
                <ul id="breadcrumbs" class="breadcrumbs">
                    <li>
                        <a href="index.php?com=bgweb&act=capnhat<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>">
                            <span>
                                <?='Cập nhật '.$title_main?>
                            </span>
                        </a>
        ...
        <div class="formRow">
            ...
            <div class="mt10">
                <img class="max-widht" src="../thumb/<?php echo _width_thumb.'x'._height_thumb.'/1/'._upload_hinhanh_l.$item['photo']?>" alt="NO PHOTO" />
            </div>
        ...
        <div class="formRow lang_hidden lang_vi active">
            <label>Repeat:</label>
            <div class="formRight">
                <label><input type="radio" name="re_peat" title="no-repeat" value="no-repeat" <?php if($item['re_peat']=="no-repeat" ){?> checked="checked"
                    <?php } ?>/> No-repeat</label>
                <label><input type="radio" name="re_peat" title="repeat-x" value="repeat-x" <?php if($item['re_peat']=="repeat-x" ){?> checked="checked"
                    <?php } ?>/> repeat-x</label>
                <label><input type="radio" name="re_peat" title="repeat-y" value="repeat-y" <?php if($item['re_peat']=="repeat-y" ){?> checked="checked"
                    <?php } ?>/>repeat-y</label>
                <label><input type="radio" name="re_peat" title="repeat" value="repeat" <?php if($item['re_peat']=="repeat" ){?> checked="checked"
                    <?php } ?>/>repeat</label>
            </div>
            <div class="clear"></div>
        </div>
        <div class="formRow lang_hidden lang_vi active">
            ...
            <div class="formRight">
                <label>
                    <input type="checkbox" title="fixed" name="fix_bg" value="fixed" <?php if($item['fix_bg']=="fixed" ){?> checked="checked"
                    <?php } ?> />fixed
                </label>
            </div>
            <div class="clear"></div>
