...
<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
            <li><a href="index.php?com=video&act=man<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>"><span>Quản lý
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="title_name_data">
                        <a href="index.php?com=video&act=edit&id=<?=$items[$i]['id']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" class="tipS SC_bold">
                            <?=$items[$i]['ten_vi']?>
                        </a>
                    </td>
                    <?php } if ($config_images){ ?>
                    <td class="textC">
                        <a href="index.php?com=video&act=edit&id=<?=$items[$i]['id']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" class="tipS SC_bold">
                            <img style="max-width: 150px;" src="<?php echo getImgYoutube($items[$i]['links']) ?>" title="<?php echo $items[$i]['ten'] ?>">
                        </a>
                    </td>
                    <?php } if ($config_noibat){ ?>
                    <td align="center">
                        <?php if(@$items[$i]['noibat']==1){?>
                        <a href="index.php?com=video&act=man&noibat=<?=$items[$i]['id']?>&type=<?=$_GET['type']?>" title="" class="smallButton tipS" original-title="Click để ẩn">
                            <img src="./images/icons/color/tick.png" alt="">
                        </a>
                        <?php } else { ?>
                        <a href="index.php?com=video&act=man&noibat=<?=$items[$i]['id']?>&type=<?=$_GET['type']?>" title="" class="smallButton tipS" original-title="Click để hiện">
                    ...
                    <td align="center">
                        ...
                        <a href="index.php?com=video&act=man&hienthi=<?=$items[$i]['id']?>&type=<?=$_GET['type']?>" title="" class="smallButton tipS" original-title="Click để ẩn">
                            <img src="./images/icons/color/tick.png" alt="">
                        </a>
                        <?php } else { ?>
                        <a href="index.php?com=video&act=man&hienthi=<?=$items[$i]['id']?>&type=<?=$_GET['type']?>" title="" class="smallButton tipS" original-title="Click để hiện">
                            <img src="./images/icons/color/hide.png" alt="">
                        </a>
                        <?php } ?>
                    </td>
